...
void set_tcp(tcp_hdr_t *tcp, uint8_t flags)
{
    ...
    tcp->dst_port = htons(PORT); /* PORTS might need to be changed later
                           depending on destination's c code */
    tcp->flags = flags;
...
uint8_t *create_packets(const char *ip_src, const char *ip_dest,
                        uint8_t ip_protocol, const char *payload, uint8_t flags)
{
    /*
     * NOTE:
...
void print_packet_unencap(uint8_t *packet_start)
{
    ...
    /* Payload */
    char *payload =
        (char *)(packet_start + sizeof(ip_hdr_t) + sizeof(tcp_hdr_t));
    printf("------------------------------------\n");
    printf("Payload msg: %s\n", payload);
}
